  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        {/* Kiri: Avatar + Info */}
        <div className={`flex-1 flex items-start ${cardGap} min-w-0`}>
          ...
          {/* Chevron expand mobile */}
          <button
            onClick={(e) => { e.stopPropagation(); onToggleExpand(); }}
            aria-label={isExpanded ? 'Tutup detail' : 'Buka detail'}
            className={`sm:hidden self-center p-2 rounded-full transition-all duration-300 ${
              isExpanded
        ...
        {/* Kanan: Kehadiran + Chevron Desktop */}
        <div className="flex items-center gap-2 w-full sm:w-auto shrink-0">
          <div className={`flex-1 sm:flex-none grid grid-cols-3 sm:flex sm:w-[220px] gap-1.5`} role="group" aria-label={`Status kehadiran ${item.name}`}>
            {ATTENDANCE_STATUSES.map((s) => {
              ...
              return (
                <button
                  ...
                  className={`flex-1 ${isCompact ? 'py-1.5 text-[11px]' : 'py-2.5 text-xs'} rounded-xl font-bold border transition-all active:scale-95 flex items-center justify-center gap-1.5 ${
          ...
          {/* Chevron expand desktop */}
          <button
            onClick={(e) => { e.stopPropagation(); onToggleExpand(); }}
            aria-label={isExpanded ? 'Tutup detail' : 'Buka detail'}
            className={`hidden sm:flex self-center p-2 rounded-full transition-all duration-300 ${
              isExpanded
                ? 'bg-orange-50 text-orange-600 dark:bg-orange-900/30 dark:text-orange-400'
                : 'text-gray-300 hover:text-gray-500 dark:text-gray-600 dark:hover:text-gray-400'
            }`}
